...
std::vector<std::vector<playing_cards::Card>> Player::permute_all_combinations (
	std::vector<playing_cards::Card> PossibleMatching) const
{
	...
	if (!PossibleMatching.empty ())
	{
		...
		do
		{
			...
			std::transform (PossibleMatching.begin (), PossibleMatching.end (),
			                std::back_inserter (VecOfPossibleMatchingRanks),
			                [] (auto const& C) { return std::vector<playing_cards::Card> {C}; });

			std::partial_sum (VecOfPossibleMatchingRanks.begin (), VecOfPossibleMatchingRanks.end (),
                                                ...
                                                std::vector<playing_cards::Card> PossibleMoves)
{
	for (auto const& SuitCard : PossibleMoves)
	{
		...
		std::remove_copy_if (Hand.begin (), Hand.end (), std::back_inserter (OtherPossibleRanksMatchingSuit),
		                     [&SuitCard] (auto const& C)
		                     {
			                     return !(SuitCard.first == C.first && SuitCard.second != C.second);
		                     });
...
void Player::find_possible_prompted_suit_moves (playing_cards::Suit PromptedSuit, playing_cards::Rank PromptedRank,
                                                std::vector<std::vector<playing_cards::Card>>&
                                                Combinations)
{
...
void Player::display_combinations (std::vector<std::vector<playing_cards::Card>> const& Combinations)
{
	std::for_each (Combinations.begin (), Combinations.end (),
	               ...
	               {
		               auto const CombStr = std::accumulate (std::next (Comb.begin ()), Comb.end (),
		                                                     playing_cards::to_string (*Comb.begin ()),
		                                                     [] (auto RunningString, auto const& C)
		                                                     {
			                                                     return RunningString + " " + playing_cards::to_string (
				                                                     C);
		                                                     });
